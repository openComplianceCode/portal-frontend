    ...
    this.apollo
      ...
      .subscribe(({ data }) => {
        ...
        this.apollo
          ...
          .subscribe();
        if (
          this.toolResult.finishAt === undefined ||
          this.toolResult.finishAt === null ||
          this.toolResult.finishAt === ''
        ) {
          ...
          if (this.toolResult.fileCount !== 0) {
            ...
            this.remainingSeconds = Math.ceil(
              fileRemainCount / this.toolResult.tool.scanRate
            );
            this.countDown();
            let remainStep =
              this.toolResult.tool.stepNumber + 1 - this.currentStep;
            if (remainStep <= 0) {
              remainStep = 1;
            }
            nextRequestInterval =
              ((this.fileCount - this.scanedFileCount) /
                remainStep /
                this.toolResult.tool.scanRate) *
              1000;
          ...
          this.apollo
            ...
            .subscribe((res) => {
              const dataStr =
                ...
                encodeURIComponent(res.data.toolResult?.outputRawJson);
